...
import sys
# Send byte object data to stdout to be read by calling Rust program
def pass_to_stdout(detection_resp_bytes):
    # Use buffer object to write binary data to standard out
    # See "note" -> https://docs.python.org/3.6/library/sys.html#sys.stdout
    buffer_object = sys.stdout.buffer
    # Don't worry about encoding since "Only ASCII characters are permitted in bytes literals (regardless of the declared source code encoding)" ->  https://docs.python.org/3.6/library/stdtypes.html#bytes

    buffer_object.write(detection_resp_bytes)
    sys.stdout.flush()
